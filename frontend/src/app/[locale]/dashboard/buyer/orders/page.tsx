...
export default function BuyerOrdersPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        console.warn("Backend unavailable, setting empty orders:", err)
        setOrders([])
      } finally {
        setLoading(false)
      }
    ...
    localStorage.removeItem('valam_mock_orders_' + email)
    fetchOrders()
  }, [])
  ...
  const filteredOrders = orders.filter(o => 
    ...
    o.supplier?.profile?.company_name?.toLowerCase().includes(searchTerm.toLowerCase())
  )
